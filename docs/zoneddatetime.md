...
new Temporal.ZonedDateTime(0n, Temporal.TimeZone.from('America/Los_Angeles'), Temporal.Calendar.from('iso8601'));
  // => Temporal.ZonedDateTime <1969-12-31T16:00:00-08:00[America/Los_Angeles]>
...
// => Temporal.ZonedDateTime <2020-08-05T20:06:13+05:45[+05:45]>
```
...
zdt = Temporal.ZonedDateTime.from({ timeZone: 'Europe/Paris', year: 2001, month: 13, day: 1 }, { overflow: 'constrain' })
  // => Temporal.ZonedDateTime <2001-12-01T00:00:00+01:00[Europe/Paris]>
/* WRONG */ zdt = Temporal.ZonedDateTime.from({ timeZone: 'Europe/Paris', year: 2001, month: 13, day: 1 }, { overflow: 'reject' })
  // => throws RangeError
```
<!-- prettier-ignore-end -->
...
zdt.startOfDay(); // => Temporal.ZonedDateTime <2015-10-18T01:00:00-02:00[America/Sao_Paulo]>
```
...
zdt = Temporal.ZonedDateTime.from('2020-01-01T12:00-08:00[America/Los_Angeles]').startOfDay();
  // => Temporal.ZonedDateTime <2020-01-01T00:00:00-08:00[America/Los_Angeles]>
zdt = Temporal.ZonedDateTime.from('2018-11-04T12:00-02:00[America/Sao_Paulo]').startOfDay();
  // => Temporal.ZonedDateTime <2018-11-04T01:00:00-02:00[America/Sao_Paulo]>
...
zdt.with({ year: 2015, minute: 31 }); // => Temporal.ZonedDateTime <2015-12-07T03:31:00-06:00[America/Chicago]>
```
...
zdt.withPlainTime({ hour: 10 }); // => Temporal.ZonedDateTime <2015-12-07T10:00:00-08:00[America/Los_Angeles]>
time = Temporal.PlainTime.from('11:22');
zdt.withPlainTime(time); // => Temporal.ZonedDateTime <2015-12-07T11:22:00-08:00[America/Los_Angeles]>
zdt.withPlainTime('12:34'); // => Temporal.ZonedDateTime <2015-12-07T12:34:00-08:00[America/Los_Angeles]>

// easier for chaining
zdt.add({ days: 2, hours: 22 }).withPlainTime('00:00'); // => Temporal.ZonedDateTime <2015-12-10T00:00:00-08:00[America/Los_Angeles]>
```
...
zdt.withPlainDate({ year: 2000, month: 6, day: 1 }); // => Temporal.ZonedDateTime <2000-06-01T03:24:30-07:00[America/Los_Angeles]>
date = Temporal.PlainDate.from('2020-01-23');
zdt.withPlainDate(date); // => Temporal.ZonedDateTime <2020-01-23T03:24:30-08:00[America/Los_Angeles]>
zdt.withPlainDate('2018-09-15'); // => Temporal.ZonedDateTime <2018-09-15T03:24:30-07:00[America/Los_Angeles]>

// easier for chaining
zdt.add({ hours: 12 }).withPlainDate('2000-06-01'); // => Temporal.ZonedDateTime <2000-06-01T15:24:30-07:00[America/Los_Angeles]>

// result contains a non-ISO calendar if present in the input
zdt.withCalendar('japanese').withPlainDate('2008-09-06'); // => Temporal.ZonedDateTime <2008-09-06T03:24:30-07:00[America/Los_Angeles][u-ca=japanese]>
zdt.withPlainDate('2017-09-06[u-ca=japanese]'); // => Temporal.ZonedDateTime <2017-09-06T03:24:30-07:00[America/Los_Angeles][u-ca=japanese]>
...
laterDay = zdt.add({ days: 1 });
  // => Temporal.ZonedDateTime <2020-03-09T00:00:00-07:00[America/Los_Angeles]>
...
laterHours = zdt.add({ hours: 24 });
  // => Temporal.ZonedDateTime <2020-03-09T01:00:00-07:00[America/Los_Angeles]>
...
earlierDay = zdt.subtract({ days: 1 });
  // => Temporal.ZonedDateTime <2020-03-08T00:00:00-08:00[America/Los_Angeles]>
  // Note that the new offset is different, indicating the result is adjusted for DST.
...
earlierHours = zdt.subtract({ hours: 24 });
  // => Temporal.ZonedDateTime <2020-03-07T23:00:00-08:00[America/Los_Angeles]>
...
zdt1.until(zdt2);
  // => Temporal.Duration <PT202956H5M29.9999965S>
...
zdt1.until(zdt2, { smallestUnit: 'seconds' });
  // => Temporal.Duration <PT202956H5M29S>
...
jan1.until(feb1, { largestUnit: 'days' }); // => Temporal.Duration <P31D>
jan1.until(feb1, { largestUnit: 'months' }); // => Temporal.Duration <P1M>
feb1.until(mar1, { largestUnit: 'days' }); // => Temporal.Duration <P29D>
feb1.until(mar1, { largestUnit: 'months' }); // => Temporal.Duration <P1M>
jan1.until(mar1, { largestUnit: 'days' }); // => Temporal.Duration <P60D>
```
<!-- prettier-ignore-end -->
...
zdt1 = Temporal.ZonedDateTime.from('1995-12-07T03:24:30.000003500+05:30[Asia/Kolkata]');
zdt2 = Temporal.ZonedDateTime.from('2019-01-31T15:30+05:30[Asia/Kolkata]');
zdt2.since(zdt1); // => Temporal.Duration <PT202956H5M29.9999965S>
```
...
zdt.round({ smallestUnit: 'hour' });
  // => Temporal.ZonedDateTime <1995-12-07T03:00:00-08:00[America/Los_Angeles]>
// Round to an increment of a unit, e.g. half an hour:
zdt.round({ roundingIncrement: 30, smallestUnit: 'minute' });
  // => Temporal.ZonedDateTime <1995-12-07T03:30:00-08:00[America/Los_Angeles]>
...
zdt.toInstant(); // => Temporal.Instant <1995-12-07T01:24:30Z>
zdt.toPlainDateTime(); // => Temporal.PlainDateTime <1995-12-07T03:24:30>
zdt.toPlainDate(); // => Temporal.PlainDate <1995-12-07>
zdt.toPlainYearMonth(); // => Temporal.PlainYearMonth <1995-12>
zdt.toPlainMonthDay(); // => Temporal.PlainMonthDay <12-07>
zdt.toPlainTime(); // => Temporal.PlainTime <03:24:30>
```
